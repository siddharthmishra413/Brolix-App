app.controller('manageCardsCtrl', function($scope, $window, userService, $state, toastr, $timeout, $interval) {
            ...
            userService.showOfferOnCards(upgrade_card).success(function(res) {
                ...
                if (res.responseCode == 200) {
                     $interval( function(){ 
                        ...
                        {
                            var sTime = new Date(res.result[i].createdAt).getTime();
                            var eTime = new Date(res.result[i].offerTime).getTime();
                            var curDate = new Date().getTime();
                                  var now = new Date().getTime();
                                   var distance = eTime - now;
